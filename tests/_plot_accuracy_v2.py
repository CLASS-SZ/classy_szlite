...
def main():
    ...
    ax.plot(nuts_summary[:, 0], nuts_summary[:, 3], 'C0o-', lw=2, ms=6,
            label="NumPyro NUTS — median across 5 seeds")
    ...
    plt.suptitle(
        rf"Gold-standard NUTS reference: $\mu_{{P_0}}^\mathrm{{gold}} = {P0_mu_g:.2f}$,  "
        rf"$\sigma_{{P_0}}^\mathrm{{gold}} = {float(data['P0_std']):.2f}$  "
        rf"({wall_gold:.0f}\,s wall)",
        fontsize=10,
    )
    plt.tight_layout()
